    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 animate-fade-in font-sans">
      <div className="bg-white w-full max-w-2xl max-h-[85vh] rounded-2xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6 bg-slate-50">
          {/* Upload Form */}
          <form onSubmit={handleSubmit} className="p-4 bg-white border border-slate-200 rounded-xl shadow-2xs space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-[11px] font-bold text-slate-700 mb-1">Monto de la Consignación ($)</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3 py-1.5 bg-slate-50 border border-slate-300 rounded-lg text-xs font-mono font-semibold focus:outline-none focus:border-indigo-600"
                />
              </div>

              <div>
                <label className="block text-[11px] font-bold text-slate-700 mb-1">Notas / Referencia Bancaria</label>
                <input
                  type="text"
                  placeholder="Ej. Transf. Bancolombia Ref #4928"
                  value={newNotes}
                  onChange={(e) => setNewNotes(e.target.value)}
                  className="w-full px-3 py-1.5 bg-slate-50 border border-slate-300 rounded-lg text-xs focus:outline-none focus:border-indigo-600"
                />
              </div>
            </div>

            <div>
              <label className="block text-[11px] font-bold text-slate-700 mb-1">Archivo de Soporte (Imagen o PDF)</label>
              <input
                type="file"
          ...
          <div className="space-y-3">
            ...
            {filteredReceipts.length === 0 ? (
              <div className="text-center py-8 bg-white border border-dashed border-slate-300 rounded-xl text-xs text-slate-400">
              ...
              filteredReceipts.map((rc) => (
                <div
                  key={rc.id}
                  className="p-4 bg-white border border-slate-200 rounded-xl shadow-2xs flex items-center justify-between gap-4"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-xs text-slate-900">{formatCOP(rc.amount)}</span>
                        {rc.verified ? (
                          <span className="text-[10px] bg-emerald-50 text-emerald-700 border border-emerald-200 px-1.5 py-0.2 rounded font-bold flex items-center gap-0.5">
                            <CheckCircle2 className="w-3 h-3" /> Verificado
                          </span>
                        ) : (
                          <span className="text-[10px] bg-amber-50 text-amber-700 border border-amber-200 px-1.5 py-0.2 rounded font-bold flex items-center gap-0.5">
                            <Clock className="w-3 h-3" /> Pendiente
                          </span>
                        )}
                      </div>
                      <p className="text-[11px] text-slate-500 mt-0.5">{rc.fileName}</p>
                      {rc.notes && <p className="text-[10px] text-slate-400 italic mt-0.5">{rc.notes}</p>}
                    </div>
                  </div>
